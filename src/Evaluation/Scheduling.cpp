...
bool
scheduling::compareSchedulingInfo(const std::shared_ptr<ScheduleInfo> a,
...
bool
scheduling::compareSchedulingInfoESTdescending(const std::shared_ptr<ScheduleInfo> a,
		const std::shared_ptr<ScheduleInfo> b)
{
	return (a->est < b->est);
}
...
void
scheduling::printSchedule(const std::vector<std::vector<std::shared_ptr<Node>>> &processorSchedule)
{
...
void
scheduling::scheduleToTikz(const std::string &filename,
...
scheduling::createScheduleInfo(const DAG &dag, unsigned &lastUniqueId, bool verbose)
{
	...
	if (verbose)
	{
		...
		{
			std::cout << "\t" << n->n->uniqueId << "\t" << n->n->wcet << "\t" << n->n->bcet << "\t"
					<< n->est << "\t" << n->lst << "\t" << n->eft << "\t" << n->eft << std::endl;
		}
	}
...
void
scheduling::scheduleIdleTask(const int proc,
...
void
scheduling::scheduleTask(const std::shared_ptr<ScheduleInfo> &task, int proc,
		std::vector<double> &processorUsage, ScheduleInfoVec &nodesToSched, ScheduleInfoVec &ready,
...
double
scheduling::computeDelta(const std::vector<double> &processorUsage,
		const ScheduleInfoVec &nodesToSched, const ScheduleInfoVec &ready, const double t,
		const double epsilon)
{
	double delta = 1000;
	...
	if (!nodesToSched.empty())
		delta = std::isless(delta, nodesToSched.back()->est - t) ?
				delta : nodesToSched.back()->est - t;
...
bool
scheduling::scheduleDAG(const DAG &dag, const unsigned nProc, const std::string &filename,
		const bool verbose)
{
	unsigned lastUniqueId;
	...
	for (double t = 0; t < dag.getPeriod() && !(ready.empty() && nodesToSched.empty()); t += delta)
	{
		//create ready queue
		...
		for (size_t i = 0; i < nProc; i++)
		{
			if (std::islessequal(processorUsage[i], epsilon))
			{
				if (!ready.empty())
				{
					...
					scheduleTask(taskChosen, i, processorUsage, nodesToSched, ready,
							processorSchedule, t, epsilon);
				}
				else
				{
					//schedule idel task
