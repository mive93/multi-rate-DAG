...
bool
compareSchedulingInfos(const ScheduleInfos& a, const ScheduleInfos& b)
{
			...
			delta)
	{
		...
		for (size_t i = 0; i < n_proc; i++)
		{
			if (std::abs(processor_usage[i]) <= epsilon)
			{
				...
				{
					if (processors_schedule[i].empty()
							|| processors_schedule[i].back()->groupId != 888)
					{
